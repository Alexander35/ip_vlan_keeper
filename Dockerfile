...
ENTRYPOINT ln -s /ip_vlan_keeper/ip_vlan_keeper_nginx.conf /etc/nginx/sites-enabled/ && rm -rf /etc/nginx/sites-enabled/default && /etc/init.d/nginx start && python manage.py makemigrations --noinput && python -u manage.py migrate --noinput && python -u manage.py createfirstuser && uwsgi --ini ip_vlan_keeper_uwsgi.ini --daemonize /ip_vlan_keeper/uwsgi.log && tail -f /dev/null
